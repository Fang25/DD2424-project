...
# Hyper Parameters
total_data=3000
learning_rate = 0.01    # learning rate                     changeable
n_steps = 30            # LSTM steps                        changeable
n_inputs = 309           # input neurons                  changeable  !! : n_steps*n_inputs=9270
n_hiddens = 200            # hidden neurons                  changeable
n_layers = 2            # LSTM layers                       changeable
n_classes = 10          # output layers（number of label）
_batch_size = 100       # batch_size                        changeable
n_iterations=2500     # number of iterations               changeable
# data
train_x = np.load('trd.npy')
train_y = np.load('trl.npy')
test_x = np.load('ted.npy')
test_y = np.load('tel.npy')
...
with tf.name_scope('inputs'):
    ...
    keep_prob = tf.placeholder(tf.float32, name='keep_prob_input')           # keep dropout
    batch_size = tf.placeholder(tf.int32, [], name='batch_size_input')       # batch size
...
with tf.Session() as sess:
    ...
    for i in range(n_iterations):
        ...
        idx = np.random.randint(total_data, size=_batch_size)
        batch_x = train_x[idx, :]
        batch_y = train_y[idx, :]
        sess.run(train_op, feed_dict={x:batch_x, y:batch_y, keep_prob:0.5, batch_size:_batch_size})
        print('step :',i, "Testing Accuracy:", sess.run(accuracy, feed_dict={x:test_x, y:test_y, keep_prob:1.0, batch_size:test_x.shape[0]}))
        if (i + 1) % 100 == 0:
